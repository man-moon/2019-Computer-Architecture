...
static int process_instruction(unsigned int instr)
{
	...
	if (opcode == 0) {
		...
		signed int tmp = instr << 21;
		signed int shamt = (signed int)tmp >> 27;
	...
	unsigned int rs = (instr << 6) >> 27;
	unsigned int rt = (instr << 11) >> 27;
	signed int tmp = instr << 16;
	signed int imm = (signed int)tmp >> 16;
	unsigned int imm2 = (instr << 16) >> 16;
	if (opcode == 0x08) {//addi
		registers[rt] = registers[rs] + imm;
		return 1;
	}
	if (opcode == 0x0c) {//andi
		registers[rt] = registers[rs] & imm2;
		return 1;
	}
	if (opcode == 0x0d) {//ori
		registers[rt] = registers[rs] | imm2;
	...
	if (opcode == 0x04) {//beq
		if (registers[rs] == registers[rt]) pc = pc + 4 * imm;
		return 1;
	}
	if (opcode == 0x05) {//bne
		if (registers[rs] != registers[rt]) pc = pc + 4 * imm;
...
static int load_program(char * const filename)
{
	...
	int idx = INITIAL_PC;
	while (fgets(inst, sizeof(inst), input)) {
		int tmp = strtoimax(inst, NULL, 0);
		memory[idx++] = tmp >> 24;
		memory[idx++] = tmp >> 16;
		memory[idx++] = tmp >> 8;
		memory[idx++] = tmp;
...
static int run_program(void)
{
	...
	while (result == 1) {
		...
		tmp += memory[pc + 3];
		pc += 4;
		result = process_instruction(tmp);
	}
	return 0;
}
